...
import { ToolMovers, type Mover } from "@/components/tool-movers";
import { PracticalSearch } from "@/components/practical-search";
...
export default async function ToolsPage({ params }: ToolsPageProps) {
  ...
  const movers: Mover[] = brief.moversJson
    ? (JSON.parse(brief.moversJson) as Mover[])
    : [];
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-10">
      ...
      {/* What's surging — GitHub + OpenRouter movers */}
      {movers.length > 0 && (
        <div className="mt-8" id="movers">
          <ToolMovers movers={movers} />
        </div>
      )}
      ...
      <div className="mt-16 mb-8 text-center">
        ...
        <p className="text-[13px] text-muted-foreground/40">
          Practical AI · Tools and workflows to solve real business problems
        </p>
      </div>
    </div>
  );
