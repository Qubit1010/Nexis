...
- **Developer Advisor** (`.claude/skills/developer-advisor/`) — The research-backed technical brain for any Web/AI/Mobile project. Two engines: **Project Architect** (marquee) turns a vague idea or client brief into a full build blueprint — runs a structured intake (asks the smallest high-leverage question set via AskUserQuestion), then outputs problem statement, recommended architecture, a cited stack table (frontend/backend/API/database/AI layer/auth/hosting), data-model sketch, how the pieces connect, a best-practices checklist, build milestones (mapped to team/agentic workflow), and risks; and **Technical Advisor** gives direct cited answers to any SE decision (architecture patterns, framework/DB/API choice, RAG-vs-agents, agentic-coding technique, PWA-vs-native, testing/CI-CD/security, hosting). Built research-first on a curated corpus of **226 2026 sources** (Exa-gathered across 9 topics + authoritative-domain boost — Fowler, Anthropic eng, OWASP/DORA, vendor docs — mirrored to a NotebookLM notebook with a live-query fallback). `references/stack-scoreboard.md` is the default-load decision layer; `research-synthesis.md` (Q1-Q9) is the cited evidence; `[sN]` citations resolve via `_research/sources.json`. Biases to the simplest thing that ships and to Aleem's stack (Next.js/Neon/Drizzle/Better Auth/Inngest/Resend/Vercel, Claude-first AI). Pre-project decision layer only — hands off to `senior-architect`/`senior-backend`/`senior-frontend` for implementation code, `claude-advisor` for Claude-product questions, `ml-expert` for model training, and `ai-use-case-generator`/`proposal-generator`/`sales-playbook` for the client pitch. Say "I want to build X", "what stack for X", "how would I architect X", "monolith or microservices", "which database", "REST or GraphQL or tRPC", "RAG or agents", "PWA or native", "where should I deploy", "review my stack", or paste a project/client brief.
