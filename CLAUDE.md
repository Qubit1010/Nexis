...
- **Leads to CRM** (`.claude/skills/leads-to-crm/`) — The current outreach lead router. Pushes manually-scraped or skill-sourced leads from the per-channel "Instant ... Leads" sheets into the matching "NexusPoint ... Outreach CRM" with a personalized Touch 1 message (OpenAI gpt-5.4-mini primary / Claude Haiku fallback). Identity-based dedup (@handle / LinkedIn `/in/slug` / Facebook group-post resolution / phone-email-domain fallback for Maps) so it never drops genuinely-new rows or duplicates sent ones — the two bugs that plagued the old lead-gen pipeline. Channel-config-driven, four channels live (Instagram, LinkedIn, Facebook, Google Maps), idempotent (re-running pushes nothing new). `--reconcile-status` self-heals stale "Added" tags against real CRM membership — use it whenever a CRM sheet ID changes. Say "push leads to CRM", "sync my instagram leads", "run the linkedin push", "dedup the CRM", "any new leads to push". Replaces the archived cold-outreach / linkedin-outreach / instagram-outreach skills + lead-gen pipeline (see `archives/outreach-skills-2026-06-18/`)
...
- **Lead Generator** (`.claude/skills/lead-generator/`) — The Google Maps *sourcing* step that feeds `leads-to-crm`. Given a manually-scraped "Instant Google Maps Data" sheet (Company Name/Category/Rating/Experience/Number/Note/Website Link), resolves each business's real Instagram/LinkedIn/Facebook profile via WebSearch (one combined query per business; a second owner/founder-specific query for LinkedIn, since it needs a person, not a company page) — agent-in-the-loop, not a headless script, since WebSearch has no callable API. A secondary free pass (`scripts/scrape_socials.py`) scrapes the business's own website footer/contact/about page for whatever WebSearch missed. Also cleans phone numbers (keeps only `+`-prefixed) and excludes South-Asia-geography rows before spending any resolution effort. Writes resolved profiles into the matching `leads-to-crm` Instant sheet (or the Google Maps/email fallback channel if nothing resolves) *and* back onto the source sheet itself (Instagram/LinkedIn/Facebook Link columns + a Social Search Status column) so results are visible in one place. Replaced an earlier Apify/SocialCrawl/Hunter.io build (archived, `archives/lead-generator-apify-2026-07-10/`) after live testing showed Exa can't index Instagram/Facebook at all and Apify's hashtag scraping produced low-quality, wrong-geography leads. Say "resolve socials for these maps leads", "find their instagram/linkedin/facebook", "enrich the google maps leads", "process the maps sheet".
...
- **Developer Advisor** (`.claude/skills/developer-advisor/`) — The research-backed technical brain for any Web/AI/Mobile project. **Problem-first, no house stack:** it understands the problem/strategy/requirements deeply, then derives the objectively best-fit architecture and stack for THAT problem from research — it never reuses another project's stack or a default. Two engines: **Project Architect** (marquee) turns a vague idea or client brief into a full build blueprint — structured intake (smallest high-leverage question set via AskUserQuestion), then problem statement, recommended architecture, a cited stack table (each row justified from the problem), data-model sketch, how the pieces connect, best-practices checklist, build milestones (agentic spec→plan→build→review; team names only if asked), and risks; and **Technical Advisor** gives direct cited answers to any SE decision (architecture, framework/DB/API, RAG-vs-agents, agentic coding, PWA-vs-native, testing/CI-CD/security, hosting). **Three-tier knowledge resolution:** local refs → NotebookLM notebook → **Exa live research on the specific problem** when the corpus is thin (`references/research-fallback.md`). Built research-first on **226 2026 sources** (Exa across 9 topics + authoritative-domain boost — Fowler, Anthropic eng, OWASP/DORA, vendor docs — mirrored to a NotebookLM notebook). `stack-scoreboard.md` is the problem→best-fit lookup; `research-synthesis.md` (Q1-Q9) is the cited evidence; `[sN]` resolves via `_research/sources.json`; `dev-context.md` is light realism-only context (no stack mandate). Pre-project decision layer only — hands off to `senior-architect`/`senior-backend`/`senior-frontend` for implementation code, `claude-advisor` for Claude-product questions, `ml-expert` for model training, and `ai-use-case-generator`/`proposal-generator`/`sales-playbook` for the client pitch. Say "I want to build X", "what stack for X", "how would I architect X", "monolith or microservices", "which database", "REST or GraphQL or tRPC", "RAG or agents", "PWA or native", "where should I deploy", "review my stack", or paste a project/client brief. **Pairs with `nexis-builder`:** developer-advisor plans (the blueprint), `nexis-builder` builds it.
- **Nexis Builder** (`.claude/skills/nexis-builder/`) — The implementation counterpart to `developer-advisor`. Advisor plans, Builder builds. Takes a developer-advisor blueprint (or a raw brief / Upwork post, in which case it calls the advisor first), then autonomously builds a complete, running project by orchestrating existing skills through a `.builder/` artifact bus: **Phase 0** developer-advisor (blueprint) → **1** senior-architect (buildable spec: complexity tier, full API contract, schema, structure) → **2** ui-ux-pro-max + taste-skill + ui-design-system (product-specific design direction, the anti-AI-slop gate) → **3-4** senior-frontend + senior-backend (build against contract + design) → **5** Test Design (trophy/pyramid, critical-path tests for auth/payments/core loop) → **6** code-reviewer (contract + security + build verification, one fix-loop) then ponytail/impeccable (YAGNI polish) → **7** completion (how to run). **Inherits the advisor's doctrine: problem-first, NO house stack** — the stack is read from the blueprint, never a default (this is the fix over the old Nexus-Fullstack-Developer pipeline, which hardcoded React+Vite+Express). Stays research-grounded during the build via the advisor's three-tier resolution (local refs → NotebookLM → Exa live) for version-sensitive "how do I best implement X in 2026" calls. Complexity-scaled, resume-safe (`status.md` is the source of truth), builds into `projects/<Name>/` by default (or an external/gitignored path for client work). Runs autonomously except ONE human checkpoint after the design direction. Say "build this project", "implement this blueprint", "turn this blueprint into code", "build me a <app>", "code this up", "run the builder", or after a blueprint "now build it".
