---
name: claude-advisor
description: "The go-to, research-backed guide to everything Claude. Explains how Claude works, the differences between the surfaces - Claude.ai chat (Projects, Artifacts), Claude Code, and Claude Cowork - plus Desktop, mobile and the API, and gives a 'which one is best for this task' decision framework. Covers Claude Code intricacies and productivity (hooks, MCP, subagents, skills, plugins, slash commands, Agent SDK), business and creative use cases, the ecosystem (best plugins, MCP servers, GitHub tools, marketplaces), building with the Claude API, model selection (Opus vs Sonnet vs Haiku), the Free/Pro/Max/Team/Enterprise plans, and the second-brain / AI memory / AI OS topic (Obsidian, Graphify, LLM Wiki/Karpathy, the 5 levels of an AI second-brain, agency setup). Use whenever Aleem or a routed client question asks anything about Claude: 'what's the difference between Claude chat and Claude Code', 'which Claude should I use for X', 'can I build this workflow in Claude Code', 'Opus vs Sonnet vs Haiku for this', 'what is Claude Cowork', 'how are businesses using Claude', 'best Claude Code plugins / MCP servers', 'how do I get more productive with Claude Code', 'which Claude plan should I buy', 'how does Claude compare to ChatGPT/Gemini', 'explain Claude to a client', 'build the free Claude guide / lead magnet', 'what is a Claude second-brain', 'how do I set up Obsidian with Claude', 'what is Graphify', 'how do I make Claude remember everything', 'build an AI OS for my agency'. Use it even when the user doesn't say the word Claude but is clearly asking which AI surface or model fits a task, or wants a comprehensive explainer of Claude's capabilities. For deep API/SDK reference specifics it hands off to claude-api; for granular Claude Code feature mechanics it defers to the claude-code-guide agent."
...
Sibling skills/agents: **claude-api** skill (deep API/SDK reference), **claude-code-guide** agent (exhaustive Claude Code mechanics), **ai-use-case-generator** (turn Claude capabilities into a client pitch), **proposal-generator** (full proposals), **sales-playbook** (the reference implementation this skill mirrors).
