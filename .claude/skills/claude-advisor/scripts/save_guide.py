#!/usr/bin/env python3
"""Save a Claude guide / advisory output as a formatted Google Doc via gws CLI.

Reads plan JSON from stdin, creates a Google Doc with proper formatting
(headings, body text, bullets, tables), and saves it to the NexusPoint Guides
folder in Google Drive. Used by the claude-advisor skill to export the
comprehensive Claude guide (and, later, the lead-magnet version).

Usage:
    echo '{"title":"...","sections":[...]}' | python save_guide.py
    cat guide.json | python save_guide.py
...
DRIVE_FOLDER_NAME = "NexusPoint Guides"
...
def find_or_create_folder():
    """Find or create the NexusPoint Guides folder in Google Drive."""
...
def create_doc(title, folder_id):
    """Create a blank Google Doc and move it to the guides folder."""
    result = run_gws(
...
def main():
    try:
        ...
        # and reads plain UTF-8 otherwise, so this works from any shell.
        sys.stdin.reconfigure(encoding="utf-8-sig", errors="replace")
        raw = sys.stdin.read()
        if not raw.strip():
            error_exit("No input received on stdin. Pipe guide JSON to this script.")
