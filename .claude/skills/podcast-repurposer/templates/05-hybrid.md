...
**Also read:** `references/creative-feedback-log.md` (editorial rules from the client's creative
team — these win over everything below when they conflict) and, for source citations behind the
2026 platform rules, `references/social-writing-research-2026.json`.
...
- **No filler tag-ons.** "Here is the difference." / "Let me explain." add nothing — cut them
  or replace with a tighter "Here's how."
- **Colon-merge choppy two-liners** when it reads better: "One habit that kills fixable deals:
...
**Reco markers:** after scoring, tag your top pick `(recommended)` and runner-up `(2nd option)`
on the hook list. The client's editor picks from a shortlist, not a flat list of five.
...
|---------|----------|-----|--------|
| A | Instagram | Teaching or save-bait | Punchy hook line / numbered list or short stacked paragraphs / ONE CTA (save trigger: "Save this for the next time X happens"). One idea per line. First line ≤125 characters (the "more" cutoff). Scannable on a phone. ~80-120 words. |
...
- **Second-sentence test on every opener:** if sentence 2 is the stronger hook, cut sentence 1
  or swap them. Scene-setters don't open.
- **Quote-first:** when a transcript quote is the anchor, the quote leads and the attribution
  follows ("Most deals are not dead. They're unstable." Then: who said it, why it matters).
- **One CTA per caption.** Stacked asks halve conversion. Pick the one action that matches the
  caption's job (save / comment / send).
...
- Optimize for **dwell time** (61s+ dwell = ~13x engagement). Comments now carry ~15x the
  algorithmic weight of a like — the closing question is a reach lever, not decoration.
...
**Every long-form post ends with an engagement CTA** (both platforms): a specific,
low-barrier, experience-based question the reader can answer in one sentence and feels
qualified to answer. "What's the toughest deal situation you've navigated?" — never
"Thoughts?" or "What do you think?". Either-or questions ("Which of these two has cost you
more?") lower the friction further.

**Formatting rules (from editorial review):**
- **Number enumerated steps.** If the post lists a named sequence (the five steps), write
  1. 2. 3. — not a comma string.
- **ALL CAPS named framework words** where the client spells them out: WHO, WHAT, WHERE,
  WHEN, WHY.
- **One light personality beat** per set is welcome where the voice file's candor supports it
...
### Step 6 — Flow pass (read-aloud edit)

Before QA, read every piece aloud once and edit for spoken rhythm. This is the pass the
client's editor was doing by hand — do it here instead:

- **Softeners where the speaker would use them:** "They're **just** unstable." "It's usually
  **just** stuck." Match how the client actually talks, not written-English stiffness.
- **Possessive reframes:** "The deal isn't the problem. **Your** response to the deal is." —
  pull the reader into the sentence.
- **Parallel structure:** "When you're irritated. When you're trying to prove a point.
  Breathe first." — repeated frames, then the turn.
- **Simplify clunk:** any sentence you stumble over reading aloud gets rewritten shorter.
- **Trim dead words:** stray "to", "that", "actually" that slow the line down.

### Step 7 — QA before writing the file
