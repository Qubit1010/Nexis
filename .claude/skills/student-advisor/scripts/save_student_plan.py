#!/usr/bin/env python3
"""Save a student plan (roadmap, study plan, career plan) as a formatted Google Doc via gws CLI.

Reads plan JSON from stdin, creates a Google Doc with proper formatting
(headings, body text, bullets, tables), and saves it to a Drive folder.

The target Drive folder defaults to "Student Advisor" and can be overridden with
the STUDENT_DOCS_FOLDER environment variable, so this stays generic/publishable.

Usage:
    echo '{"title":"...","sections":[...]}' | python save_student_plan.py
    cat plan.json | python save_student_plan.py
...
def create_doc(title, folder_id):
    """Create a blank Google Doc and move it to the target folder."""
    result = run_gws(
