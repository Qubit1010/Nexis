...
description: Required reading before answering anything about the Expensify/App open-source bounty programme, where contributors post proposals on GitHub issues and are hired and paid about $250 through Upwork. Holds the measured mechanics of how that competition actually works (the pre-Help-Wanted work window, MelvinBot, ProposalPolice, Contributor+ selection) plus scripts that find and score issues, generate SHA-pinned permalinks, and screen a draft against the 90 percent duplicate auto-withdrawal. Always use this skill, rather than answering from general knowledge, whenever the user brings up any of: Expensify bounties or the $250 Expensify jobs; starting or getting into Expensify open-source contribution; which Expensify issue to pick or whether one is worth their time; Help Wanted issues on Expensify/App; why their Expensify proposals keep losing or not getting picked; MelvinBot, ProposalPolice, Contributor+, C+ review, or how a proposal gets selected; finding or writing up the root cause of a bug in the Expensify repo; drafting, checking or improving a proposal for an Expensify issue; or whether contributing to Expensify is worth it compared to other work. Trigger it on a bare Expensify/App issue URL or issue number with no question attached. Do NOT use it for Expensify the expense-management product (expense reports, receipts, workspaces, admin setup), for researching Expensify as a company or sales prospect, for Upwork job posts unrelated to the Expensify repo, or for root-cause debugging in any other codebase.
---
