"""
brain-sync — Two-way mirror between Nexis and the agency-brain Obsidian vault.
...
Everything else (wiki/, raw/, clients/, CRMs, .env) never syncs.

Modes:
  python sync_vault.py --check              Drift report, both directions, no writes
  python sync_vault.py --push               Nexis -> vault (+ decisions union merge)
  python sync_vault.py --pull               Vault -> Nexis (+ decisions union merge)
...
def ingest_status() -> dict:
    """How far the vault wiki lags behind Nexis knowledge. Falls back to
...
def cmd_check(as_json: bool = False) -> int:
    ...
    for rel in sorted(set(nexis) & set(vault)):
        if file_md5(nexis[rel]) != file_md5(vault[rel]):
            newer = "Nexis" if nexis[rel].stat().st_mtime > vault[rel].stat().st_mtime else "vault"
    ...
    if not (nexis_only or vault_only or differ):
        print("In sync — vault matches Nexis for the scoped subset.")
        return 0
    if nexis_only:
        print(f"Nexis-only ({len(nexis_only)}) — missing in vault (push to copy):")
        for rel in nexis_only:
            print(f"  {rel}")
    if vault_only:
        print(f"Vault-only ({len(vault_only)}) — missing in Nexis (pull to copy):")
        for rel in vault_only:
            print(f"  {rel}")
    if differ:
        print(f"Differ ({len(differ)}):")
        for rel, newer in differ:
            side = nexis if newer == "Nexis" else vault
            print(f"  {rel}  (newer: {newer}, {fmt_mtime(side[rel])})"
                  + ("  [union merge on push/pull]" if rel == DECISIONS_KEY else ""))
    print("\nRun --push (Nexis -> vault) or --pull (vault -> Nexis). decisions/log.md always union-merges.")
    return 1
...
def main():
    ap = argparse.ArgumentParser(description="Nexis <-> agency-brain vault sync")
    ap.add_argument("--check", action="store_true", help="Drift report, no writes")
    ap.add_argument("--push", action="store_true", help="Nexis -> vault")
    ap.add_argument("--pull", action="store_true", help="Vault -> Nexis")
